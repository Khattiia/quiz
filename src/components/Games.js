...
const Games = () => {
  ...
  async function fetchGamesHandler() {
    setLoading(true);
  ...
  function goToNextQuestion() {
    if (currentQuestionIndex + 1 < games.length) {
      ...
      setCorrectAnswer(games[currentQuestionIndex + 1].correct_answer);
      combineAllAnswers(
        games[currentQuestionIndex + 1].incorrect_answers,
        games[currentQuestionIndex + 1].correct_answer
      );
    } else {
      setShowScore(true);
    }
  ...
  return (
    <div>
      <Navigation />
      <h1 className={classes.h1}>Video Games Quiz</h1>
      {showScore ? (
        <Score
          points={currentPoints}
          totalQuestions={games.length}
          retryQuiz={retryQuiz}
        />
      ) : (
              ...
              {games.length > 0 ? (
                <div>
                  <div className={classes.box}>
                    {removeCharacters(games[currentQuestionIndex].question)}
                  </div>
                  <br />
