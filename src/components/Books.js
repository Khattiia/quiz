...
const Books = () => {
  ...
  function goToNextQuestion() {
    if (currentQuestionIndex + 1 < books.length) {
      setCurrentQuestionIndex(currentQuestionIndex + 1);
      setCorrectAnswer(books[currentQuestionIndex + 1].correct_answer);
      combineAllAnswers(
        books[currentQuestionIndex + 1].incorrect_answers,
        books[currentQuestionIndex + 1].correct_answer
      );
    } else {
      setShowScore(true);
    }
  ...
  return (
    <div>
      <Navigation />
      <h1 className={classes.h1}>Books Quiz</h1>
      {showScore ? (
        <Score
          points={currentPoints}
          totalQuestions={books.length}
          retryQuiz={retryQuiz}
        />
      ) : (
              ...
              {books.length > 0 ? (
                <div>
                  <div className={classes.box}>
                    {removeCharacters(books[currentQuestionIndex].question)}
                  </div>
                  <br />
